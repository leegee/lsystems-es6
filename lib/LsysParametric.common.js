...
if ( 1 == 1 ) {
	LOGGER = {};
	LOGGER.trace = LOGGER.log = LOGGER.warn = LOGGER.debug = LOGGER.info = function () {};
}
if ( 1 == 12 || !console ) {
	console = {};
...
exports.prototype.render = function () {
	...
	for ( var i = 0; i < this.content.length; i++ ) {
		...
		case 'g':
			...
			i += ( "" + this.generation )
				.length;
			LOGGER.log( '---> Set Generation ' + this.generation );
			break;
		case 'f':
...
exports.prototype.turtle_graph = function ( dir ) {
	...
	if ( this.options.time_scale_lines > 0 ) {
		...
		// LOGGER.log( 'this.stepped=%d,  LINE WIDTH: %d', this.stepped, this.ctx.lineWidth )
