...
var exports = module.exports = function Lsys(options) {
    ...
    this.penWidth = this.options.max_line_width;
	this.variables = null;
	this.interpolateVarsRe = null;

	this.initialize();
    this.max_x = this.min_x = this.options.init_x || 0;
    this.max_y = this.min_y = this.options.init_y || 0;
    this.x = this.options.init_x || 0;
    this.y = this.options.init_y || this.options.canvas_height / 2;
	this.y -= this.options.turtle_step_y;
    this.content = '';
...
exports.prototype.render = function () {
	...
	for (var i = 0; i < this.content.length; i++) {
		...
		// Start a branch
		case '[':
			states.push([ dir, this.x, this.y, this.colour, this.penWidth-1, this.stepped ]);
			draw = false;
            // console.info(this.penWidth);
            if (this.penWidth < 1) this.penWidth = 1;
            this.setWidth( this.penWidth );
			break;
		// End a branch
		case ']':
			...
			this.y = state[ 2 ];
			this.colour = state[ 3 ];
            this.penWidth = state[ 4 ];
			this.stepped = state[ 5 ];
			draw = true;
			break;
		};
