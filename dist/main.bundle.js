...
var _Ctrls = __webpack_require__(329);

document.addEventListener('DOMContentLoaded', function () {
	new _Ctrls.Ctrls().init();
}, {
	passive: true
});
...
var Ctrls = exports.Ctrls = function () {
    function Ctrls() {
        _classCallCheck(this, Ctrls);
    }
    ...
    _createClass(Ctrls, [{
        ...
        key: "loadPreset",
        value: function loadPreset(idx) {
            console.log('Load preset ', idx, _presets.Presets[idx]);
            Object.keys(_presets.Presets[idx]).forEach(function (i) {
                console.log('set preset key %s to %s', i, _presets.Presets[idx][i]);
                try {
                    var el = document.getElementById(i);
                    if (el.nodeName === 'INPUT') {
                        el.setAttribute('value', _presets.Presets[idx][i]);
                    } else if (el.nodeName === 'TEXTAREA') {
                        el.innerText = _presets.Presets[idx][i];
                    }
                } catch (e) {
            ...
            document.getElementById('title').innerText = _presets.Presets[idx].title;
        }
    }, {
        key: "installPresets",
        value: function installPresets() {
            var _this = this;

            var ul = document.getElementById('presets');
            console.log('List presets', _presets.Presets);
            _presets.Presets.forEach(function (i, j) {
                var li = document.createElement('li');
                li.innerText = i.title;
                li.dataset.presetNumber = j;
                li.addEventListener('click', function (e) {
                    _this.loadPreset(e.target.dataset.presetNumber);
                }, {
                    passive: true
                });
                ul.appendChild(li);
        ...
        key: "setupEventListerners",
        value: function setupEventListerners() {
            var _this2 = this;

            this.generate.addEventListener('click', function (e) {
                console.log('Generate');
                _this2.generate.value = 'Generating...';
                _this2.generate.disabled = true;
                _this2.createMidi.disabled = true;

                var options = {};
                Object.keys(_presets.Presets[0]).forEach(function (i) {
                    try {
                ...
                options.canvas = _this2.el_canvases.insertBefore(canvas, _this2.el_canvases.firstChild);;

                var lsys = new _LsysParametric.Lsys(options, canvas);
                lsys.generate(options.total_generations);
                if (_this2.elTimeDisplay) {
                    _this2.elTimeDisplay.innerText = 'Generated in ' + (new Date().getTime() - elTimeDisplay.get('text')) + ' ms';
                }
                ...
                _this2.contentDisplay.value = lsys.content;
                _this2.generate.value = 'Generate';
                _this2.generate.disabled = false;
                _this2.createMidi.value = 'Generate MIDI';
                _this2.createMidi.disabled = false;
            }, {
                passive: true
            });

            this.createMidi.addEventListener('click', function () {
                var oldValue = _this2.createMidi.value;
                _this2.createMidi.value = 'Hang on...';
                _this2.createMidi.disabled = true;
                fetch('/cgi-bin/fractal_plant_chords.cgi', {
                    ...
                    _this2.playSound("/cgi-output/cgi.midi");
                    _this2.createMidi.value = oldValue;
                    _this2.createMidi.disabled = false;
                }).catch(function () {
                    console.error(e);
                    alert('Failure :(');
...
var Lsys = exports.Lsys = function () {
	function Lsys(options, canvas) {
		_classCallCheck(this, Lsys);
		...
		this.options = { // rename to default
			...
			time_scale_lines: 1,
			clearCanvas: true,
			colours: ["rgba(130,  90, 70, 0.8)", "rgba(33, 180, 24, 0.6)", "rgba(50, 210, 50, 0.5)", "rgba(70, 255, 70, 0.4)"]
		};
	...
	_createClass(Lsys, [{
		...
		key: 'castVariables',
		value: function castVariables(str) {
			...
			str.split(/[\n\r\f]+/).forEach(function (line) {
				...
				if (name2val) {
					...
					// Cast
					if (rv[name2val[2]].match(/^(-+)?\d+(\.\d+)?$/)) rv[name2val[2]] = parseFloat(rv[name2val[2]]);
				} else {
  	...
  	*/
		value: function castRules(str) {
			...
			str.split(/[\n\r\f]+/).forEach(function (line) {
				if (line == '') return;
				var head_tail = line.match(/^\s*(.+?)\s*->\s*([^\n\r\f]+)\s*/);

				if (head_tail != null) {
					var match_condition = head_tail[1].match(/([^:]+)\s*:?\s*(.*?)\s*$/);
					var head = match_condition[1].match(/^(.+?)\s*$/);
					var rule = [head[1], match_condition[2], head_tail[2]];
				} else {
		...
		key: 'dsin',
		value: function dsin(radians) {
			return Math.sin(radians * RAD);
		}
	}, {
		key: 'dcos',
		value: function dcos(radians) {
			return Math.cos(radians * RAD);
		}
	}, {
		key: 'generate',
		value: function generate(generations) {
		...
		key: 'interploateVars',
		value: function interploateVars(str) {
			var _this2 = this;

			var rv = str.replace(this.interpolateVarsRe, function (match) {
				return typeof _this2.variables[match] != 'undefined' ? _this2.variables[match] : match;
			});
			console.log('Interpolate vars: %s ... %s', str, rv);
			return rv;
		}
	}, {
		key: 'string2reAndArgNames',
		value: function string2reAndArgNames(str) {
			var argNames = [];
			this.str2reRe = new RegExp(/(\w+)\(([^\)]+)\)/);

			var rv = str.replace(this.str2reRe, function (match, varname, argsCsv) {
			...
			return [new RegExp(rv, 'g'), argNames];
		}
	}, {
		key: 'apply_rules',
		value: function apply_rules() {
			var _this3 = this,
			...
			atoms.forEach(function (atom) {
				...
				_this3.options.rules.forEach(function (rule) {
					ruleNumber++;
					...
					var _ = _this3.string2reAndArgNames(rule[0]);
					var rule2findRe = _[0];
					...
					var atomAfterRuleApplied = atom.replace(rule2findRe, function (original) {
						/*  On entering this function, a match has been found but rules have yet to be tested */
						// Ascribe variables
						for (var i = 2; i < _arguments.length - 2; i++) {
							console.log("Let " + ruleArgNames[i - 2] + ' = ' + _arguments[i]);
							// Set variables with values found
							_this3.variables[ruleArgNames[i - 2]] = _arguments[i];
						}

						// Get the rule code:
						var ruleConditionJs = _this3.interploateVars(rule[1]);
						console.log('Rule ' + ruleNumber + ' condition: ' + ruleConditionJs);

						// Decide if the substitution take place
						var ruleConditionMet = ruleConditionJs.length == 0 ? true : eval(ruleConditionJs);
						...
						var substituted = _this3.interploateVars(rule[2]);
			...
			console.log('# FINAL for generation ' + this.generation + '/' + this.total_generations + ' ############################ Content: ' + this.content);
		}
	}, {
		key: 'render',
		...
		key: 'turtleGraph',
		value: function turtleGraph(dir) {
		...
		key: 'setWidth',
		value: function setWidth(px) {
			this.ctx.lineWidth = px;
		}
	}, {
		key: 'setColour',
		value: function setColour(index) {
		...
		value: function resize() {
			...
			var wi = this.min_x < 0 ? Math.abs(this.min_x) + Math.abs(this.max_x) : this.max_x - this.min_x;
			var hi = this.min_y < 0 ? Math.abs(this.min_y) + Math.abs(this.max_y) : this.max_y - this.min_y;
